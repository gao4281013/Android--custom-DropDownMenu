...
public class FixedTabIndicator extends LinearLayout {
    ...
    private void switchTab(String needp,int pos) {
        ...
        }else if("1".equals(needp)){


            if (mOnItemClickListener != null) {
                mOnItemClickListener.onItemClick(tv, pos, tv.getCurrentTextColor()== mTabDefaultColor ? false:true);
            }
            if (mLastIndicatorPosition == pos) {
                // 点击同一个条目时
                tv.setTextColor(tv.getCurrentTextColor()== mTabDefaultColor ? mTabSelectedColor : mTabDefaultColor);
                return;
            }
     ...
     */
    private View generateTextView(Map<String,String> title, int pos) {
        // 子空间TextView
        final String needpic = title.get("needPic");
        TextView tv = new TextView(context);
        tv.setGravity(Gravity.CENTER);
        tv.setText(title.get("title"));
